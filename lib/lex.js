...
export class Token {
    ...
    toString() {
        return `${this.type}: ` + JSON.stringify(this.content) + `@${this.position}`
    }
}
class TokenMatch {
    /**
        ...
        if(!this._possible) {
            this._possible = {
                ...
                none: [
                    ...
                    [new RegExpTokenMatch(/^[$][a-zA-Z]\w*/, 2), "varname"],
        ...
        if(!this._tokens) {
            ...
            while(contents) {
                ...
                if(last_contents == contents) {
                    throw new Error(`Lexer loop in mode ${mode} @${position} with ${JSON.stringify(last_contents.substring(0, 15) + "...")}`)
                }
                if(contents == "") {
                    break
                }
