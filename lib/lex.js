...
class TokenFinderRecursive extends TokenFinder {
    ...
    constructor(type, v, m) {
        ...
        this.finalEndMatch = v.finalEndMatch
...
class TokenFinderConfig {
    ...
    tokensFrom(contents, position, parent_state = undefined) {
        ...
        const final_end_marker = parent_state && parent_state.finder.finalEndMatch
        ...
        while(contents.length > position.offset) {
            ...
            if(end_marker && contents.startsWith(end_marker, old_offset)) {
                ...
                return chunks
            } else if(final_end_marker && contents.startsWith(final_end_marker, old_offset)) {
                chunks.push(new Token("$pop", final_end_marker, position.immutableCopy))
                position.consume(final_end_marker)
                const rest = contents.substring(position.offset)
                if(rest) {
                    chunks.push(new Token("$other", final_end_marker, position.immutableCopy))
                    position.consume(rest)
                }
                if(process.env.debug) {
                    console.log("" + chunks[chunks.length - 1])
                }
