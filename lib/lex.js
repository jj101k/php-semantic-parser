...
class TokenFinderRecursive extends TokenFinder {
    ...
    parse(contents, position) {
        ...
        if(tokens.length) {
            const start_marker = contents.substring(old_offset, position.offset)
            return [
                ...tokens,
                ...this.then.tokensFrom(contents, start_marker, position, this),
            ]
        } else {
            return []
        }
...
class TokenFinderConfig {
     ...
     * @param {MutablePosition} position
     * @param {TokenFinderRecursive | undefined} parent_token
     * @return {Token[]}
     */
    tokensFrom(contents, start_marker, position, parent_token = undefined) {
        const abort_marker = parent_token ? parent_token.abortMatch : null
        const end_marker = parent_token && parent_token.endMatch && start_marker ?
            parent_token.finder.endMatchGiven(parent_token.endMatch, start_marker) :
            start_marker
        /**
         * @type {Token[]}
         */
