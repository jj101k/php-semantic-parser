...
class TokenFinderRecursive extends TokenFinder {
    ...
    parse(contents, position) {
        ...
        if(tokens.length) {
            ...
            return [
                ...tokens,
                ...this.then.tokensFrom(contents, end_match, position),
            ]
        } else {
            return []
        }
...
class TokenFinderConfig {
     ...
     * @return {Token[]}
     */
    tokensFrom(contents, end_marker, position) {
        /**
         * @type {Token[]}
         */
        const chunks = []
        while(contents.length > position.offset) {
            ...
            if(position.offset == old_offset) {
                throw new Error(`Lexer failed to find a match @${position} with ${JSON.stringify(contents.substring(old_offset, old_offset + 15) + "...")}`)
            }
        }
        return chunks
    }
