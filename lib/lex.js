...
class TokenFinderString extends TokenFinder {
     ...
     */
    matches(s, position) {
        if(s.substring(position.offset, position.offset + this.value.length) == this.value) {
            return this.value
        } else {
            return null
        }
...
class TokenFinderRecursive extends TokenFinder {
    ...
    parse(contents, position) {
        ...
        if(tokens.length) {
            const t = contents.substring(old_offset, position.offset)
            const end_match = this.endMatch ?
                this.finder.endMatchGiven(this.endMatch, t) :
                t
            return [
                ...tokens,
                ...this.then.tokensFrom(contents, end_match, position, this.abortMatch),
            ]
        } else {
            return []
        }
...
class TokenFinderConfig {
     ...
     * @param {MutablePosition} position
     * @param {string | undefined} abort_marker
     * @return {Token[]}
     */
    tokensFrom(contents, end_marker, position, abort_marker) {
        /**
         * @type {Token[]}
         */
        const chunks = []
        while(contents.length > position.offset) {
            const old_offset = position.offset
            if(end_marker && contents.substring(old_offset, old_offset + end_marker.length) == end_marker) {
                ...
                return chunks
            } else if(abort_marker && contents.substring(old_offset, old_offset + abort_marker.length) == abort_marker) {
...
export class Lex {
    ...
    get tokens() {
        if(!this._tokens) {
            this._tokens = Lex.finder.tokensFrom(this.contents, null, new MutablePosition())
        }
        return this._tokens
    }
}
