...
export class ParserStaticExpression extends ParserBase {
     ...
     */
    static endOfExpressionParser(next_mode = new ParserStateHandlerEnd()) {
...
export class ParserExpression extends ParserStaticExpression {
    ...
    get initialMode() {
        const binary_boolean_operator = c => {
            const assignment = new this.handlerClasses.BooleanOperator(c, this.popNodeForRead())
            this.nodes.push(assignment)
            return new ParserStateChange(assignment)
        }
        const maybeArray = new ParserStateHandler({
        ...
        const postArgument = new ParserStateHandler({
            arrow: () => {
                const concatenation = new ParserPropertyReference(this.popNodeForRead())
                this.nodes.push(concatenation)
                return new ParserStateChange(concatenation)
            },
            equals: () => {
            ...
            openBracket: () => {
                const n = new ParserFunctionCall(this.popNodeForRead())
                this.nodes.push(n)
                return new ParserStateChange(n)
            },
            openCurly: () => {
                console.warn("Use of curlies for array indices (eg. $foo{\"bar\"}) is deprecated")
                const n = new ParserCurlyArrayMemberRef(this.popNodeForRead())
                this.nodes.push(n)
                return new ParserStateChange(n)
            },
            openSquare: () => {
                const n = new ParserArrayMemberRef(this.popNodeForRead())
                this.nodes.push(n)
                return new ParserStateChange(n)
            },
            plusplus: () => {
            ...
            questionMark: () => {
                const n = new ParserTernary(this.popNodeForRead())
                this.nodes.push(n)
                return new ParserStateChange(n)
            },
            ...this.commentOrSpace,
            ...
            bareword_and: binary_boolean_operator,
            bareword_instanceof: () => {
                const test = new ParserInstanceof(this.popNodeForRead())
                this.nodes.push(test)
                return new ParserStateChange(test)
            },
            bareword_or: binary_boolean_operator,
            bareword_xor: binary_boolean_operator,
            ampersand: c => {
                const assignment = new this.handlerClasses.BitOperator(c, this.popNodeForRead())
                this.nodes.push(assignment)
                return new ParserStateChange(assignment)
            },
            bitOperator: c => {
            ...
            dot: () => {
                const concatenation = new this.handlerClasses.StringConcatenation(this.popNodeForRead())
                this.nodes.push(concatenation)
                return new ParserStateChange(concatenation)
            },
            doubleQuestionMark: () => {
                const c = new this.handlerClasses.CoalesceOperator(this.popNodeForRead())
                this.nodes.push(c)
                return new ParserStateChange(c)
            },
            doubleColon: () => {
                const concatenation = new ParserClassValueReference(this.popNodeForRead())
                this.nodes.push(concatenation)
                return new ParserStateChange(concatenation)
            },
            inPlaceConcatenation: () => {
            ...
            ...ParserComparison.expressionParser(() => this.popNodeForRead(), p => this.nodes.push(p)),
    ...
    popNodeForRead() {
        ...
        if(last instanceof ParserVariable && !this.namespace.has(last.name)) {
            console.warn(`Variable ${last.name} is used before it's defined`)
        }
        return last
    }
}
