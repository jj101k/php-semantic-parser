...
export class ParserAnyBlock {
     ...
     */
    static lineModes(f, next_mode = "entry") {
        return {
            else: {
                bareword_if: () => {
                ...
                openCurly: () => {
                    ...
                    f(node)
                    return {consumer: node, mode: next_mode, reconsumeLast: 1}
                },
                space: () => {},
            },
            entry: {
                ...
                openCurly: () => {
                    const b = new ParserBlock()
                    f(b)
                    return {consumer: b, mode: next_mode, reconsumeLast: 1}
                },
            },
            lineEnd: {
                semicolon: () => ({mode: next_mode}),
            },
            postIf: {
                bareword: () => ({mode: next_mode, reconsumeLast: 1}),
    ...
    static generalModes(f) {
        const line_modes = this.lineModes(f)
        return {
            ...line_modes,
            entry: {
                ...line_modes.entry,
