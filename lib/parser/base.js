...
export class ParserBase {
    ...
    parse(tokens, filename) {
        ...
        for(let i = 0; i < tokens.length; i++) {
            ...
            if(handler) {
                try {
                    const r = handler(t.content, t, filename)
                    if(r) {
                        if(r.reconsumeLast) {
                            if(!r.consumer && !r.mode) {
                                throw new InternalError("Cannot reconsume without some kind of change")
                            }
                            i-=r.reconsumeLast
                        }
                        if(r.consumer) {
                            r.consumer.setParentNamespace(this.namespace)
                            const consumed = r.consumer.parse(tokens.slice(i + 1), filename)
                            i += consumed
                        }
                        if(r.mode) {
                            mode = r.mode
