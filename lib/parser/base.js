...
export class ParserBase {
    ...
    /**
     * @type {{[mode: string]: ParserStateHandler, initial: ParserStateHandler}}
     */
    get modes() {
        return {
    ...
    get nope() {
        return new ParserStateChange(null, "end", 1)
    }
    /**
     *
     * @param {Token[]} tokens
    ...
    parse(tokens) {
        ...
        for(let i = 0; i < tokens.length; i++) {
            ...
            const exact_handler = this.modes[mode].handlers[`${t.type}_${t.content.toLowerCase()}`]
            const token_handler = this.modes[mode].handlers[t.type]
            const handler = exact_handler || token_handler || this.modes[mode].handlers["$else"]
            if(handler) {
                ...
                throw new InternalError(
                    `Unexpected node of type ${t.type} (${t.content}) in state ${mode}${position_info}\n` +
                    `Supported are: ${Object.keys(this.modes[mode].handlers)}`)
            }
        }
        this.onEOF(tokens)
